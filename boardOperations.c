...
void createBoard(int boardSize, struct slot **upLeft, struct slot **upRight, struct slot **downLeft, struct slot **downRight,struct players record[],int no_players){
	...
	for(int i =0; i< boardSize; i++){
		...
		for(int j=0;j < boardSize; j++){
			...
			//printf("\n (%d , %d)= %s ",board[i][j].row,board[i][j].column,board[i][j].slot_name);
	...
	{
		record[i].player_slot=reachDesiredElement(rand()%7,rand()%7,upLeft);
	}*/
	//*********************************
	...
	for(int i = 1; i < boardSize -1; i++){
		...
		//printf("||||up (%d, %d)",board[0][0].up->row,board[0][0].up->column);
		//printf("||||left (%d, %d)",board[0][0].left->row,board[0][0].left->column);
	...
	*downRight = &board[boardSize -1][boardSize -1];
	...
	//Assigning players to slots-------------------------------------
		for(int i=0;i<no_players;i++)
		{
			...
			record[i].player_slot=&board[row][column];
		}
